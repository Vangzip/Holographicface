...
bool texturemeshPoisson::getCropHullMesh(pcl::PolygonMesh &outmeshfile){
    ...
    const size_t meshPointCount =
        static_cast<size_t>(m_polygonmeshptr.cloud.width) *
        static_cast<size_t>(m_polygonmeshptr.cloud.height);

    const size_t convertedPointCount = poisson_point->points.size();
    const size_t pointCountForCheck =
        meshPointCount > 0 ? meshPointCount : convertedPointCount;

    int maxCropHullOutIndex = -1;
    for (size_t i = 0; i < m_crophulloutpointindex.size(); ++i)
    {
        if (m_crophulloutpointindex[i] > maxCropHullOutIndex)
        {
            maxCropHullOutIndex = m_crophulloutpointindex[i];
        }
    }

    cout << COUT_PREFIX << "[DEBUG-CROPHULL] mesh_point_count="
        << meshPointCount
        << ", converted_point_count=" << convertedPointCount
        << ", polygon_count=" << m_polygonmeshptr.polygons.size()
        << ", crophull_out_index_count=" << m_crophulloutpointindex.size()
        << ", crophull_out_index_max=" << maxCropHullOutIndex
        << endl;

    size_t invalidFaceCount = 0;
    size_t invalidVertexCount = 0;
    size_t shortFaceCount = 0;
    size_t globalMaxFaceVertexIndex = 0;
    bool hasFaceVertex = false;

    // Set to false if the console output is too large.
    const bool dumpEveryFaceMaxVertex = true;

    for (size_t faceIndex = 0; faceIndex < m_polygonmeshptr.polygons.size(); ++faceIndex)
    {
        const pcl::Vertices& vertices = m_polygonmeshptr.polygons[faceIndex];
        ...
        size_t faceMaxVertexIndex = 0;
        bool faceHasVertex = false;
        bool faceInvalid = false;

        for (size_t vertexIndex = 0; vertexIndex < vertices.vertices.size(); ++vertexIndex)
        {
            const size_t index = static_cast<size_t>(vertices.vertices[vertexIndex]);

            if (!faceHasVertex || index > faceMaxVertexIndex)
            {
                faceMaxVertexIndex = index;
                faceHasVertex = true;
            }

            if (!hasFaceVertex || index > globalMaxFaceVertexIndex)
            {
                globalMaxFaceVertexIndex = index;
                hasFaceVertex = true;
            }

            if (pointCountForCheck > 0 && index >= pointCountForCheck)
            {
                ++invalidVertexCount;
                faceInvalid = true;
            }
        ...
        if (dumpEveryFaceMaxVertex)
        {
            cout << COUT_PREFIX << "[DEBUG-CROPHULL] face="
                << faceIndex
                << ", vertex_count=" << vertices.vertices.size()
                << ", max_vertex_index=" << (faceHasVertex ? faceMaxVertexIndex : 0)
                << ", invalid=" << (faceInvalid ? 1 : 0)
                << endl;
        }
    }

    cout << COUT_PREFIX << "[DEBUG-CROPHULL] global_max_face_vertex_index="
        << (hasFaceVertex ? globalMaxFaceVertexIndex : 0)
        << ", invalid_face_count=" << invalidFaceCount
        << ", invalid_vertex_count=" << invalidVertexCount
        << ", short_face_count=" << shortFaceCount
        << endl;

    if (pointCountForCheck > 0 && hasFaceVertex && globalMaxFaceVertexIndex >= pointCountForCheck)
    {
        cout << COUT_PREFIX << "[DEBUG-CROPHULL] ERROR: face vertex index out of range. max_vertex_index="
            << globalMaxFaceVertexIndex
            << ", point_count=" << pointCountForCheck
            << endl;
    }
    ...
    */
    ...
    cout << COUT_PREFIX << "[DEBUG-CROPHULL] visible_faces="
        << visibleFaces.size()
        << ", removed_faces="
        << (m_polygonmeshptr.polygons.size() - visibleFaces.size())
        << endl;
   ...
   */
    ...
    cout << COUT_PREFIX << "[DEBUG-CROPHULL] before simplify: cloud_point_count="
        << static_cast<size_t>(visible.cloud.width) * static_cast<size_t>(visible.cloud.height)
        << ", polygon_count=" << visible.polygons.size()
        << endl;
   ...
   */
    ...
    cout << COUT_PREFIX << "[DEBUG-CROPHULL] after simplify: cloud_point_count="
        << static_cast<size_t>(outmeshfile.cloud.width) * static_cast<size_t>(outmeshfile.cloud.height)
