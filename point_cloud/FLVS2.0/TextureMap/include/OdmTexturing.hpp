...
{
    int index;//相机的id
    double angle;//该点所在面的法线与对应相机光轴的夹角的衡量标准
    int x;//在该相机中的水平坐标
    int y;//在该相机中的垂直坐标

    inline bool operator == (const POINTVISIBLE& b)//判断是否为同一个点
    {
        return index == b.index;
    }

    inline bool operator < (const POINTVISIBLE& b)//按index,angle升序排列
    {
        if (index == b.index)
...
{
    int index;
    double angle; //面的法线与对应相机光轴的夹角的衡量标准
    string maskfile;
    pcl::PointXY coords[3];
    int x[3];//三个点在该相机中的水平坐标
    int y[3];//三个点在该相机中的垂直坐标

    inline bool operator < (const VISIBLE& b)
    {
        return angle < b.angle;
    }
};
...
struct Patch
{
    ...
    std::vector<pcl::PointXY> facesPointXY_;
...
private:
    ...
    void getMatForFace(const Patch &, const Patch & dstPatch, cv::Mat &srcout, cv::Mat &dstout);

    void replaceFace(const Patch &src, Patch dest);
	...
	pcl::TextureMesh::Ptr mesh_current_;    /**< PCL Texture Mesh */
    ...
    std::vector<int> cameraAngle_;
    float angle_;// 法线与相机角度阈值
    pcl::texture_mapping::CameraVector cameras_;    /**< The vector containing all cameras. */
    std::map<size_t, pcl::PointCloud<pcl::PointXYZ>::Ptr> cameraPoseMap_;
    std::vector<std::vector<VISIBLE>> faceVisibles_;
    std::vector<std::vector<POINTVISIBLE>> pointvisibles;//每个点的可见相机
    std::vector<std::vector<VISIBLEFACE> > cameraFaces_;   //存放每个相机下的三角面
    //std::vector<Patch> objPathes_; //原模型中当前可见面

    std::vector<std::vector<Patch>> camerasPathes_;
    std::vector<std::vector<Patch>> objAllPathes_;

    std::vector<std::vector<int>> tTIAVector_;

    string _allTexture;
    cv::Mat _srcimage, _outTxture;
};
