...
$targetDir = Join-Path $holoDir "target"
$exePath = Join-Path $targetDir "Holo.exe"
$inputStem = "0"
...
function Add-PathIfExists([System.Collections.Generic.List[string]]$Paths, [string]$Path) {
...
function Find-FirstExistingFile([string[]]$Paths) {
...
function Initialize-HoloRuntime([string]$HoloDir, [string]$TargetDir) {
    $thirdPartyRoot = [System.IO.Path]::GetFullPath((Join-Path $HoloDir ".."))
    $runtimePaths = [System.Collections.Generic.List[string]]::new()

    Add-PathIfExists $runtimePaths $TargetDir
    Add-PathIfExists $runtimePaths (Join-Path $thirdPartyRoot "opencv450\opencv\build\x64\vc15\bin")
    Add-PathIfExists $runtimePaths (Join-Path $thirdPartyRoot "PCL 1.12.1-rc1\bin")
    Add-PathIfExists $runtimePaths (Join-Path $thirdPartyRoot "PCL 1.12.1-rc1\3rdParty\Boost\lib")
    Add-PathIfExists $runtimePaths (Join-Path $thirdPartyRoot "PCL 1.12.1-rc1\3rdParty\FLANN\bin")
    Add-PathIfExists $runtimePaths (Join-Path $thirdPartyRoot "PCL 1.12.1-rc1\3rdParty\OpenNI2\Redist")
    Add-PathIfExists $runtimePaths (Join-Path $thirdPartyRoot "PCL 1.12.1-rc1\3rdParty\Qhull\bin")
    Add-PathIfExists $runtimePaths (Join-Path $thirdPartyRoot "PCL 1.12.1-rc1\3rdParty\VTK\bin")
    Add-PathIfExists $runtimePaths (Join-Path $thirdPartyRoot "OSG365\bin")
    Add-PathIfExists $runtimePaths (Join-Path $thirdPartyRoot "OE32\bin")

    Add-PathIfExists $runtimePaths "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin"
    Add-PathIfExists $runtimePaths "C:\Program Files\Microsoft Visual Studio\18\Community\VC\Redist\MSVC\14.51.36231\x64\Microsoft.VC145.OpenMP"
    Add-PathIfExists $runtimePaths "C:\Program Files\Microsoft Visual Studio\18\Community\VC\Redist\MSVC\14.51.36231\onecore\x64\Microsoft.VC145.OpenMP"
    Add-PathIfExists $runtimePaths "C:\Program Files\NVIDIA Corporation\Nsight Systems 2022.1.3\host-windows-x64"
    Add-PathIfExists $runtimePaths "C:\Program Files\Polyspace\R2020b\sys\java\jre\win64\jre\bin"
    Add-PathIfExists $runtimePaths "C:\Program Files\Polyspace\R2020b\bin\win64"
    ...
    if (Test-Path -LiteralPath $TargetDir -PathType Container) {
        $knownDlls = @{
            "vcomp140.dll" = @(
                "C:\Program Files\Microsoft Visual Studio\18\Community\VC\Redist\MSVC\14.51.36231\x64\Microsoft.VC145.OpenMP\vcomp140.dll",
                "C:\Program Files\Microsoft Visual Studio\18\Community\VC\Redist\MSVC\14.51.36231\onecore\x64\Microsoft.VC145.OpenMP\vcomp140.dll",
                "C:\Program Files\Polyspace\R2020b\bin\win64\vcomp140.dll"
            )
            "zlib.dll" = @(
                "C:\Program Files\NVIDIA Corporation\Nsight Systems 2022.1.3\host-windows-x64\zlib.dll"
            )
            "msvcr100.dll" = @(
                "C:\Program Files\Polyspace\R2020b\sys\java\jre\win64\jre\bin\msvcr100.dll",
                "C:\Program Files\Polyspace\R2020b\sys\java\jre\win64\jre\bin\plugin2\msvcr100.dll"
            )
        }

        foreach ($dllName in $knownDlls.Keys) {
            $source = Find-FirstExistingFile $knownDlls[$dllName]
...
Initialize-HoloRuntime $holoDir $targetDir
