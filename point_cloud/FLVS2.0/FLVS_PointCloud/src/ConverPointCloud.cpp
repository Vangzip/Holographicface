...
bool ConverPointCloud::parseArguments(const string &config){
    ...
    {
        cout << COUT_PREFIX << "config file no find. file =" << config << endl;
        return false;
    }
    ...
    while (getline(iff, line))
    {
        ...
        else if (line.find("holesize=") != string::npos)//是否补洞
        {
...
bool ConverPointCloud::createPoissonMesh(const string &filepath){
    ...
    if (pcl::io::loadPLYFile(filepath, *cloud_normal) != 0){
    ...
    string parent = FileLibrary::getInstance()->getFileParentPath(srcfile);
    string name = FileLibrary::getInstance()->getFileNameFromPath(srcfile);

    m_strOutModelPath =srcfile.substr(0, srcfile.find(".ply")) + "_poisson_mesh.ply";
    pcl::io::savePLYFile(m_strOutModelPath, triangles);
    cout << "save mesh file : " << m_strOutModelPath << endl;

	//去掉包围盒之外的三角面
    texturemeshPoisson poissoinmesh(srcfile,m_strOutModelPath);
    poissoinmesh.setPolygonMesh(triangles);
    poissoinmesh.meshCropHull(cloud_normal);
...
bool ConverPointCloud::createGreedMesh(const string &filepath){
    ...
    m_strOutModelPath = srcfile.substr(0, srcfile.find(".ply")) + "_greed_mesh.ply";
#if 1
    //补洞
    pcl::PolygonMesh meshPoly;
    fillHole(triangles, meshPoly);    
    ...
    if (m_holesize != 0.0)
    {
        ...
        nearestKSearchNormal(*cloud);

        pcl::toPCLPointCloud2(*cloud, meshPoly.cloud);
    ...
    cout << "save mesh ply file = " << m_strOutModelPath << endl;
...
bool ConverPointCloud::meshAPI(const string &flypath, const string &config){
...
bool ConverPointCloud::normalsMovingLeastSquares( pcl::PointCloud<PointXYZRGBNormal> &mls_points){
    pcl::PointCloud<PointXYZRGBNormal>::Ptr pointPtr( new  pcl::PointCloud<PointXYZRGBNormal>);
    pcl::PointCloud<PointXYZRGBNormal> tgt;
    if (m_normalsIter1 == 0)
    {
        return true;
    }
    ...
    for (size_t i = 0; i < m_normalsIter1; i++){
        ...
        pointPtr->clear();
        pointPtr->reserve(mls_points.points.size());
        for (size_t i = 0; i < mls_points.points.size(); i++)
        {
            pointPtr->points.push_back(mls_points.points[i]);
        }
        //pointPtr->insert(pointPtr->end(), mls_points.begin(), mls_points.end());

        pcl::search::KdTree<pcl::PointXYZRGBNormal>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZRGBNormal>);
        tree->setInputCloud(pointPtr);

        pcl::MovingLeastSquares<pcl::PointXYZRGBNormal, pcl::PointXYZRGBNormal> mls;
        // Set parameters
        mls.setInputCloud(pointPtr);
        mls.setPolynomialFit(true); //对于法线的估计是有多项式还是仅仅依靠切线
        mls.setComputeNormals(false);
        mls.setSearchMethod(tree);
        mls.setPointDensity(30);
    ...
    mls_points.clear();
    mls_points.reserve(tgt.points.size());
    for (size_t i = 0; i < tgt.points.size(); i++)
    {
        mls_points.points.push_back(tgt.points[i]);
    }
...
{
	//法线重新计算
    pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr allpoint(new pcl::PointCloud<pcl::PointXYZRGBNormal>);
    allpoint->points.reserve(mls_points.points.size());
    for (size_t i = 0; i < mls_points.points.size(); i++)
    {
        allpoint->points.push_back(mls_points.points[i]);
    }
    
    pcl::search::KdTree<pcl::PointXYZRGBNormal> search;
    search.setInputCloud(allpoint);
    pcl::PointCloud<PointXYZRGBNormal>::Ptr neight(new PointCloud<pcl::PointXYZRGBNormal>);
    ...
    for (size_t i = 0; i < mls_points.points.size(); i++)
    {
        ...
        neight->clear();
        neight->points.resize(indices.size());
...
bool ConverPointCloud::nearestKSearchNormal(pcl::PointCloud<PointXYZRGBNormal> &mls_points){
    ...
    //allpoint->clear();
    allpoint->points.resize(mls_points.points.size());
    for (size_t i = 0; i < mls_points.points.size(); i++)
    {
        ...
        allpoint->points[i].a = mls_points.points[i].a;
    ...
    for (size_t iterNum = 0; iterNum < m_normalsIter2; iterNum++)
    {
        ...
        for (size_t i = 0; i < mls_points.points.size(); i++)
        {
            ...
            current_point.y = mls_points.points[i].y;
            current_point.z = mls_points.points[i].z;
            current_point.r = mls_points.points[i].r;
            current_point.g = mls_points.points[i].g;
            current_point.b = mls_points.points[i].b;
            current_point.a = mls_points.points[i].a;
            ...
            for (size_t Knum = 0; Knum < m_neighbor_num; Knum++)
            {
                ...
                tmpNormalPoint.points[i].a = mls_points.points[i].a;
